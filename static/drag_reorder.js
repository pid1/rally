...
(function () {
    ...
    function initDragReorder(config) {
        ...
        function firstTrailingChild(list, moving) {
            return (
                Array.from(list.children).find(
                    (child) => child !== moving && !(child.matches(itemSelector) && canDrag(child))
                ) || null
            );
        }
        ...
        function updateDropTarget() {
            ...
            if (drag.item.parentElement !== list || drag.item.nextSibling !== before) {
        ...
        function startEdgeScroll() {
            const step = () => {
                ...
                if (drag.pointerY < EDGE_ZONE_PX) {
                    ...
                    delta = EDGE_SPEED_MAX_PX * ((drag.pointerY - height + EDGE_ZONE_PX) / EDGE_ZONE_PX);
                }
                if (delta) {
        ...
        function cancelDrag() {
            ...
            const state = drag;
            drag = null;
            if (state.started) state.originList.insertBefore(state.item, state.originNext);
            teardown(state);
        }
